...
class MCRepriceError:
    def __init__(self, df, r, q, S_0, rho, alpha, beta, dt, device='cpu'):
        ...
        self.T_max = self.df['maturity'].max()
        self.data_type = torch.float32
        
    def run_vectorized(self, n_paths=10000):
        M = len(self.df)
        vols = torch.tensor(self.df['implied_vol'].values, dtype=self.data_type, device=self.device).view(M, 1)
        ...
        print("\n--- Calibration Results ---")
        print(self.df[['strike', 'maturity', 'price', 'reprice', 'abs_pct_error']].head())
